...
test('settings UI exposes explicit secret-safe backup and restart-staged restore controls', () => {
    ...
    assert.match(html, /id="config-restore-file"[^>]+data-handler-change=/);
    assert.match(app, /stageConfigRestore\(this\)/);
    assert.match(app, /downloadConfigBackup\(\)/);
...
test('restore UI uses the dedicated media type, explicit confirmation, and never requests an automatic restart', () => {
    const start = app.indexOf('async function stageConfigRestore');
    const end = app.indexOf('async function runReportNow', start);

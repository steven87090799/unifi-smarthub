...
const durationMs = isTestRun ? testDurationMs : Math.max(30 * 60 * 1000, Number(process.env.SOAK_DURATION_MS) || 30 * 60 * 1000);
...
async function openSoakServer(sseClients, sse) {
    const server = http.createServer((req, res) => {
        if (req.url === '/events') {
            ...
            sseClients.add(res);
            res.write(': connected\n\n'); // flush headers so the test client can prove connection lifecycle
    ...
    sse.write([...sseClients][0], 'data: connected\n\n');
...
function printSummary(summary) {
    ...
    console.log(`Timer baseline: ${summary.timerBaseline}`);
    ...
    console.log(`Unexpected handles final: ${summary.unexpectedHandles.length}`);
    if (summary.unexpectedHandles.length) console.log(`Unexpected handle types: ${summary.unexpectedHandles.join(', ')}`);
    console.log(`Unexpected requests final: ${summary.unexpectedRequests.length}`);
    if (summary.failureMessages.length) console.log(`Failures: ${summary.failureMessages.join(' | ')}`);
    console.log(`Exit code: ${summary.ok ? 0 : 1}`);
}
...
async function main() {
    ...
    const sse = createSseBackpressure({
        maxWritableLength: 32, drainTimeoutMs: 25,
        onRemove: client => { sseRemoved += 1; sseClients.delete(client); }
    });
    ...
    try {
        ...
        const cleanup = history.cleanupYielding(30, 100_000, { batchSize: 100 });
        assert.strictEqual(history.cleanupYielding(30, 100_000), cleanup, 'cleanup must coalesce concurrent runs');
        cleanupRuns += 1;
        await cleanup;
        ...
        try {
            ...
            lease.prune();
            for (const client of [...sseClients]) sse.close(client);
            sseResponse?.destroy();
            sseRequest?.destroy();
    ...
    try {
        ...
        assert.equal(timers.count(), 0, 'tracked timers remained after cleanup');
    ...
    const summary = {
        ...
        timerBaseline: 0, timerFinal: timers.count(), sseClientsFinal: sseClients.size,

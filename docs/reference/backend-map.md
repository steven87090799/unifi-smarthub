...
- WiiM `WIIM_IP` 為選配且只接受 literal IPv4/IPv6；留空時 status/history/command/art/sampler/watcher 都不觸發裝置讀取，封面代理以 numeric IPv4/IPv6（含 mapped／compatible IPv6）分類並拒絕 transition/special ranges，所有 DNS 答案都驗證後 pin，redirect 每跳重驗證，最多 4 個 upstream／16 個 queue／7 秒 deadline，並有 Trusted LAN 私有 target／明確 legacy opt-in 的 TLS/HTTP、redirect/MIME/大小與 bounded LRU 限制。
...
- UniFi Network API 對公網預設嚴格 TLS 驗證；`UNIFI_NETWORK_CA_FILE` 可指定絕對私有 CA，resolved CA bytes 由 shared TLS policy 傳給 transport，不在 request 時重讀 raw path；Trusted LAN 只對分類為私有的 target 產生 scoped compatibility policy，其他目標停用驗證與 HTTP 都必須分別明確 opt-in。
...
- PPB HTTPS 對公網預設驗證；私有 CA 只接受有界的絕對一般檔案，Trusted LAN 私有 target 可由 master switch 產生 scoped compatibility policy，其他目標停用驗證必須明確設定 `PPB_TLS_INSECURE=true`。

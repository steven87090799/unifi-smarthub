...
**取樣頻率是裝置感知的自適應模式**:前端只替目前可見頁面實際顯示的 trend／NAS／WiiM／Linux 裝置續約；後端以自己的時間給最多 3 分鐘短租約，沒有續約、切頁或分頁進背景就自動回到低頻。總覽會替它顯示的 trend、NAS、WiiM 同時續約；UPS 是唯一例外，為了斷電紀錄不做閒置降頻。

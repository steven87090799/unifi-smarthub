...
總覽或目前裝置頁可見時，相關前端資料與後端取樣使用 3 秒節奏；但 UniFi Device telemetry API 只讀專用 snapshot，Controller/Device SSH 仍固定用 60/300 秒 sampler。Snapshot 過期時，卡片以 STALE badge、橘色框與「上次成功溫度／CPU（上次成功資料）／資料已過期」清楚標示，不把最後值標成目前溫度；未設定 Host Key 的選取設備同時顯示未鎖定。切頁、背景分頁或租約到期後立即回到低頻。

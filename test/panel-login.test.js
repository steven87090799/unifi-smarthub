...
test('production and mock register the same panel auth routes and dashboard exposes logout', () => {
    assert.match(serverSource, /registerPanelAuthRoutes\(app, \{ rootDir: __dirname, security: panelSecurity \}\)/u);
    assert.match(mockSource, /registerPanelAuthRoutes\(app, \{ rootDir: __dirname, security: mockSecurity \}\)/u);
...
test('public login assets work before authentication and protected HTML redirects to login', async t => {
    ...
    registerPanelAuthRoutes(app, { rootDir: ROOT, security });
    ...
    for (const asset of ['/login', '/assets/login.css', '/js/login.js']) {

...
- **總覽**:4 張 KPI 卡(WAN 狀態→雲端頁、線上設備→客戶端頁、24H 威脅→資安頁、CPU 溫度→硬體區塊)+ 歷史趨勢圖(24h/7d 切換)+ UCG 硬體詳情。
- **客戶端**:管理表格 + Top 5 流量排行榜 + 封鎖歷史時間軸。
- **總覽 → 資安戰情速覽**:安全評分環(0-100 + 等級)、24H 每小時威脅柱狀圖、最新攔截事件流,點「進入完整戰情室」跳資安頁。
- **資安**(完整):6 格 KPI 列(評分/24H攔截/總數/高危/來源國/封鎖中)、自動防禦聯動開關、每小時堆疊柱狀圖(依類別)、威脅世界地圖、戰情室、Top 來源 IP/目標設備/連接埠三欄分析、事件表(時間+搜尋+危險度+類別四重篩選、CSV 匯出、來源 IP 複製)。

...
**取樣頻率是裝置感知的自適應模式**:UCG／NAS／WiiM／UPS／AdGuard／Linux 設備頁可見時，該頁全部前端資訊每 3 秒更新；對應的後端歷史取樣也會切到 3 秒。切頁會立即撤銷上一個設備 scope，分頁進背景也主動釋放；心跳意外中斷時仍有伺服器短租約保護，租約到期即回到各項原本預設。總覽維持預設輪詢，避免同時高頻查詢所有設備；UPS 的斷電歷史取樣獨立持續運作，不因頁面狀態降頻。
